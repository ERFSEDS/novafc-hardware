...
FlightSimulator::FlightSimulator(Configuration& config, RocketData &data, SensorValues &sensors, std::string inputFile, std::string outputFile) : 
		config(config),
		rocket(data), 
		sensors(sensors),
		gyroNoiseMean(0), 
		...
		accelNoiseSTD(0),
		state{config}, 
		brain{config, this->state, data, sensors, &armCallback, this, &fireCallback, this},
		armed(false)
 {
